...
def get_inline_moves_dict(player: str, board: Dict[Tuple[int, int, int], str], empty_positions: Set[Tuple[int, int, int]]) -> List[Move]:
    ...
    for direction, group in groups:
        ...
        # CASE 2: Destination is occupied by an opponent => potential push.
        elif dest in board and board[dest] == opponent:
            ...
            for i in range(1, 3):
                ...
                if next_dest in board and board[next_dest] == opponent:
                    opponent_positions.append(next_dest)
                elif next_dest in board and board[next_dest] == player:
                    no_adjacent = False
                    break
                else:
                    break
...
def get_side_step_moves_dict(player: str, board: Dict[Tuple[int, int, int], str], empty_positions: Set[Tuple[int, int, int]]) -> List[Move]:
    ...
    for inline_direction, group in groups:
        ...
        for side_direction in get_side_step_directions(inline_direction):
            ...
            if all((pos[0], pos[1], pos[2]) in empty_positions for pos in dest_positions):
                move_obj = Move(
                    player=player,
...
def get_next_turn_colour(player_colour: str) -> str:
    """
    Returns the next player to moves colour. This can be used to get the opponent's marble colour.
